...
Green Dev Dashboard is a software sustainability observatory for CI/CD workflows. It collects metrics from GitHub Actions, estimates energy consumption and CO2 emissions, stores the data in SQLite, and presents repository-level insights in an interactive dashboard.

The project uses a simplified Software Carbon Intensity inspired model:

```text
CO2 = workflow duration x estimated runner energy use x carbon intensity
```

The current model is intentionally transparent. It estimates CI energy from workflow duration and a fixed carbon factor, making the assumptions easy to inspect and improve.
...
Check the backend API endpoint:

```bash
curl http://localhost/api/green-metrics
```
...
Receives metrics from GitHub Actions.
...
Returns recent metrics for the dashboard.
...
The app can be deployed on one small AWS EC2 instance with Docker Compose.
